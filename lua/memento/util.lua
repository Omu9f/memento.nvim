...
function M.invert(color)
  ...
  elseif type(color) == "string" then
    local hsluv = require("memento.hsluv")
    if color ~= "NONE" then
...
---@param groups memento.Highlights
...
function M.cache.file(key)
  return vim.fn.stdpath("cache") .. "/memento-" .. key .. ".json"
end

---@param key string
function M.cache.read(key)
  ---@type boolean, memento.Cache

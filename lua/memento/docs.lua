...
local Groups = require("memento.groups")
...
function M.extras()
  local Extra = require("memento.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/memento/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/memento/config.lua", "\n(--@class memento%.Config.-\n})")

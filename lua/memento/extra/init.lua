...
function M.setup()
  ...
  -- memento.setup({ plugins = { all = true } })
  for _, extra in ipairs(names) do
    local info = M.extras[extra]
    local plugin = require("memento.extra." .. extra)
    for style, style_name in pairs(styles) do
      local colors, groups, opts = memento.load({ style = style, plugins = { all = true } })
      local fname = extra
      ...
      colors["_upstream_url"] = "https://github.com/folke/memento.nvim/raw/main/extras/" .. fname
      colors["_style_name"] = "Tokyo Night" .. style_name
      colors["_name"] = "memento_" .. style
      colors["_style"] = style
      print("[write] " .. fname)
